    ...
    <div className="max-w-md w-full space-y-6 bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-lg">
      <div className="text-center space-y-2">
        ...
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">
          Sign In to BIM Academy
        </h1>
        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
          Access your courses, live cohorts, and certificates
        </p>
      </div>

      {error && <ErrorState message={error} title="Authentication Error" />}
...
export default function LoginPage() {
  return (
    <div className="min-h-[calc(100vh-12rem)] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <Suspense
        fallback={
          <div className="max-w-md w-full p-8 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 animate-pulse text-center text-xs text-slate-500">
